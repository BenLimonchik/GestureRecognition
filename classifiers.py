...
def complex_model1(X,y,is_training):
    ...
    Wconv2 = tf.get_variable("Wconv2", shape=[5, 5, 64, 32])
    bconv2 = tf.get_variable("bconv2", shape=[32])
    Wconv3 = tf.get_variable("Wconv3", shape=[3, 3, 32, 32])
    bconv3 = tf.get_variable("bconv3", shape=[32])
    W1 = tf.get_variable("W1", shape=[31552, 1024])
    b1 = tf.get_variable("b1", shape=[1024])
    W2 = tf.get_variable("W2", shape=[1024, 512])
    b2 = tf.get_variable("b2", shape=[512])
    W3 = tf.get_variable("W3", shape=[512, 128])
    ...
    h11 = tf.nn.relu(a11)
    print(h11.shape)
    h11_norm = tf.contrib.layers.batch_norm(h11, center=True, scale=True, is_training=is_training)
    
    # third conv layer (with only relu)
    a3 = tf.nn.conv2d(h11_norm, Wconv3, strides=[1,1,1,1], padding="VALID") + bconv3
    h3_norm = tf.nn.relu(a3)
    
    # pool layer
    h3 = tf.layers.max_pooling2d(inputs=h3_norm, pool_size=[2, 2], strides=2) 
    print('h3',h3.shape)
    # three affine layers
    h3_flat = tf.reshape(h3,[-1,31552])
    h4 = tf.matmul(h3_flat,W1) + b1
    h5 = tf.nn.relu(h4)
